"""
NVR_sequences_multi_element_independent.py
=========================
Non-Verbal Reasoning — Sequences Item Generator
Standalone Module: Multi-Element Independent
"""
...
TARGET_SUBTYPE = "multi_element_independent"
...
class NVRSequenceGenerator:
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    def _gen_multi_element_independent(self, tier):
        cfg        = TIERS[tier]
        complexity = cfg["complexity"]
        n_shown    = N_FRAMES_SHOWN[tier]

        n_elems   = 2 if complexity<=4 else 3
        has_decoy = (complexity>=4 and random.random()<0.4)
        shapes    = random.sample(self.all_shapes, min(n_elems,len(self.all_shapes)))

        size_seq  = [0.18,0.28,0.38,0.48,0.58]
        rot_step  = random.choice([45,60,90])
        pos_seq = [
            [(0.25,0.35),(0.45,0.35),(0.65,0.35),(0.65,0.55),(0.65,0.70)],
            [(0.75,0.65),(0.60,0.65),(0.45,0.65),(0.30,0.65),(0.22,0.65)],
            [(0.50,0.50)]*5,
        ]
        elem_rules = []
        for k in range(n_elems):
            if k==0:            r="size_grow"
            elif k==1:          r=random.choice(["rotate","position","size_shrink"])
            else:               r="fill_alternate"
            if has_decoy and k==n_elems-1: r="decoy"
            elem_rules.append(r)
        fills_base = ["solid","hollow","solid"]

        def _elem_state(k, fi):
            shape = shapes[k%len(shapes)]
            rule  = elem_rules[k]
            if rule=="decoy":
                return {"shape":shape,"size_frac":0.22,"rotation":0,
                        "fill":fills_base[k],"pos":(0.78,0.22)}
            if rule=="size_grow":
                return {"shape":shape,"size_frac":size_seq[min(fi,len(size_seq)-1)],
                        "rotation":0,"fill":fills_base[k],
                        "pos":pos_seq[0][min(fi,len(pos_seq[0])-1)]}
            elif rule=="size_shrink":
                return {"shape":shape,
                        "size_frac":size_seq[min(max(0,len(size_seq)-1-fi),len(size_seq)-1)],
                        "rotation":0,"fill":fills_base[k],
                        "pos":pos_seq[1][min(fi,len(pos_seq[1])-1)]}
            elif rule=="rotate":
                return {"shape":shape,"size_frac":0.32,
                        "rotation":(fi*rot_step)%360,"fill":fills_base[k],
                        "pos":pos_seq[1][min(fi,len(pos_seq[1])-1)]}
            elif rule=="position":
                return {"shape":shape,"size_frac":0.28,"rotation":0,
                        "fill":fills_base[k],
                        "pos":pos_seq[0][min(fi,len(pos_seq[0])-1)]}
            else:  # fill_alternate
                return {"shape":shape,"size_frac":0.22,"rotation":0,
                        "fill":"solid" if fi%2==0 else "hollow",
                        "pos":pos_seq[2][0]}

        def _make_indep_frame(fi):
            return {"subtype":"multi_element_independent",
                    "elements":[_elem_state(k,fi) for k in range(n_elems)]}

        frames = [_make_indep_frame(fi) for fi in range(n_shown+1)]
        answer_state = frames[-1]; shown_frames = frames[:-1]

        correct = answer_state
        d1 = _make_indep_frame(n_shown-1)
        d2 = copy.deepcopy(correct)
        d2["elements"][min(1,n_elems-1)] = _elem_state(min(1,n_elems-1),n_shown-1)
        d3 = copy.deepcopy(correct)
        d3["elements"][0] = _elem_state(0,0)
        d4 = copy.deepcopy(correct)
        rule0 = elem_rules[0]
        if rule0=="size_grow":
            d4["elements"][0]["size_frac"] = size_seq[max(0,min(n_shown,len(size_seq)-1)-n_shown)]
        else:
            d4["elements"][0]["size_frac"] = size_seq[min(n_shown+1,len(size_seq)-1)]

        def _emergency(existing):
            for step in range(1,n_shown+3):
                cand = _make_indep_frame(step%(n_shown+2))
        ...
        return {
            "subtype":"multi_element_independent","frames":shown_frames,"answer":answer_state,
            "distractors":distractors,
            "distractor_strategy":"all_behind+elem1_wrong+elem0_stuck+elem0_reversed",
            "tier":tier,"b_estimate":b_est,"age_range":cfg["age_range"],
            "n_rules":n_elems,
            "rule_axes":"+".join(f"e{k}={r}" for k,r in enumerate(elem_rules)),
            "shape_types":"|".join(shapes),"fill_levels":"mixed",
            "has_secondary":has_decoy,"has_hatch":False,"is_multilayer":True,"is_subcell":False,
        }

    def _fallback_multi_element_independent(self, tier):
        cfg=TIERS[tier]; n_shown=N_FRAMES_SHOWN[tier]
        fracs=[0.18,0.28,0.38,0.48,0.58]
        def _mf(fi): return {"subtype":"multi_element_independent","elements":[
            {"shape":"circle","size_frac":fracs[min(fi,len(fracs)-1)],
             "rotation":0,"fill":"solid","pos":(0.30,0.50)},
            {"shape":"square",
             "size_frac":fracs[min(max(0,len(fracs)-1-fi),len(fracs)-1)],
             "rotation":0,"fill":"hollow","pos":(0.70,0.50)},]}
        frames=[_mf(fi) for fi in range(n_shown+1)]
        answer_state=frames[-1]; shown_frames=frames[:-1]
        distractors=_dedup_options(answer_state,[
            _mf(n_shown-1),_mf(n_shown+1),_mf(0),_mf(n_shown-2)
        ],lambda ex:_mf(n_shown+2))
        return {"subtype":"multi_element_independent","frames":shown_frames,"answer":answer_state,
                "distractors":distractors,"distractor_strategy":"fallback",
                "tier":tier,"b_estimate":round(random.uniform(*cfg["b_range"]),2),
                "age_range":cfg["age_range"],"n_rules":2,"rule_axes":"e0=grow+e1=shrink",
                "shape_types":"circle|square","fill_levels":"mixed",
                "has_secondary":False,"has_hatch":False,"is_multilayer":True,"is_subcell":False}

    # ═══════════════════════════════════════════════════════════════════════════
    # TARGET RENDERER
    # ═══════════════════════════════════════════════════════════════════════════
    def _render_indep_frame(self, canvas, state):
        fw,fh=canvas.width,canvas.height; half=self.frame_hd//2
        for elem in state["elements"]:
            px=int(elem["pos"][0]*fw); py=int(elem["pos"][1]*fh)
            r=int(elem["size_frac"]*half)
            self._draw_shape(canvas,elem["shape"],elem["fill"],
                             px,py,r,elem.get("rotation",0))

    # ═══════════════════════════════════════════════════════════════════════════
    # DISPATCH ROUTINES
    # ═══════════════════════════════════════════════════════════════════════════
    def generate_item(self, tier):
        return _safe_gen(self._gen_multi_element_independent, self._fallback_multi_element_independent, tier)
    ...
    def render_frame(self, state, show_question_mark=False):
        ...
        self._render_indep_frame(canvas, state)
        return canvas
