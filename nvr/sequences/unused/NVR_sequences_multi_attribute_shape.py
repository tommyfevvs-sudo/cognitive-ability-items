"""
NVR_sequences_multi_attribute_shape.py
=========================
Non-Verbal Reasoning — Sequences Item Generator
Standalone Module: Multi-Attribute Shape
"""
...
TARGET_SUBTYPE = "multi_attribute_shape"
...
class NVRSequenceGenerator:
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    def _gen_multi_attribute_shape(self, tier):
        cfg        = TIERS[tier]
        complexity = cfg["complexity"]
        n_shown    = N_FRAMES_SHOWN[tier]

        shape      = random.choice(["circle","square","triangle","pentagon"])
        n_attrs    = min(complexity, 3)
        attr1_rule = random.choice(
            ["grow","shrink"] if complexity<=3
            else ["grow","shrink","oscillate","accelerate"])
        step_mag   = ({"small":0.08,"medium":0.13,"large":0.20}
                      [random.choice(["large"] if complexity<=2
                                     else ["medium","large"] if complexity<=4
                                     else ["small","medium"])])

        base = 0.18
        size_seq = [round(base+i*step_mag,3) for i in range(8)]
        size_seq = [s for s in size_seq if 0.12<=s<=0.78]
        if len(size_seq)<4: size_seq=[0.18,0.30,0.42,0.54,0.66]

        fill_states = {
            "binary_alternate": ["hollow","solid"]*6,
            "three_cycle":      ["hollow",("lines","diagonal_tl_br"),"solid"]*4,
        }
        attr2_key    = "binary_alternate" if complexity<=3 else "three_cycle"
        attr2_seq    = fill_states[attr2_key]
        rot_step_deg = 45 if complexity<=4 else 30

        def _size_at(fi):
            if attr1_rule=="grow":
                return size_seq[min(fi, len(size_seq)-1)]
            elif attr1_rule=="shrink":
                return size_seq[min(max(0, len(size_seq)-1-fi), len(size_seq)-1)]
            elif attr1_rule=="oscillate":
                period = (len(size_seq)-1)*2
                phase  = fi % period
                idx    = phase if phase < len(size_seq) else period-phase
                return size_seq[max(0,min(idx,len(size_seq)-1))]
            else:  # accelerate
                acc = min(sum(range(1,fi+2)), len(size_seq)-1)
                return size_seq[acc]

        def _make_attr_frame(fi):
            return {"subtype":"multi_attribute_shape","shape":shape,
                    "size_frac":_size_at(fi),
                    "fill":attr2_seq[fi%len(attr2_seq)] if n_attrs>=2 else "hollow",
                    "rotation":(fi*rot_step_deg)%360 if n_attrs>=3 else 0}

        frames = [_make_attr_frame(fi) for fi in range(n_shown+1)]
        answer_state = frames[-1]; shown_frames = frames[:-1]

        correct = answer_state
        d1 = _make_attr_frame(n_shown-1)
        d2 = copy.deepcopy(correct); d2["fill"]=attr2_seq[(n_shown-1)%len(attr2_seq)]
        d3 = copy.deepcopy(correct); d3["size_frac"]=_size_at(max(0,n_shown-2))
        d4 = copy.deepcopy(correct)
        d4["rotation"] = (correct["rotation"]-rot_step_deg*2) % 360

        def _emergency(existing):
            for step in range(1,8):
                cand = _make_attr_frame((n_shown+step)%(n_shown+4))
        ...
        return {
            "subtype":"multi_attribute_shape","frames":shown_frames,"answer":answer_state,
            "distractors":distractors,
            "distractor_strategy":"size_behind+fill_wrong+size_wrong+rot_reversed",
            "tier":tier,"b_estimate":b_est,"age_range":cfg["age_range"],
            "n_rules":n_attrs,
            "rule_axes":(f"size_{attr1_rule}({step_mag})+{attr2_key}"
                         +(f"+rot_{rot_step_deg}" if n_attrs>=3 else "")),
            "shape_types":shape,"fill_levels":attr2_key,
            "has_secondary":False,"has_hatch":isinstance(attr2_seq[0],tuple),
            "is_multilayer":False,"is_subcell":False,
        }

    def _fallback_multi_attribute_shape(self, tier):
        cfg=TIERS[tier]; n_shown=N_FRAMES_SHOWN[tier]
        fracs=[0.18,0.30,0.42,0.54,0.66]; fills=["hollow","solid"]*6
        def _mf(fi): return {"subtype":"multi_attribute_shape","shape":"circle",
                              "size_frac":fracs[min(fi,len(fracs)-1)],
                              "fill":fills[fi%len(fills)],"rotation":0}
        frames=[_mf(fi) for fi in range(n_shown+1)]
        answer_state=frames[-1]; shown_frames=frames[:-1]
        distractors=_dedup_options(answer_state,[
            _mf(n_shown-1),_mf(n_shown+1),
            {**_mf(n_shown),"fill":"solid" if answer_state["fill"]=="hollow" else "hollow"},
            {**_mf(n_shown),"size_frac":fracs[max(0,min(n_shown-2,len(fracs)-1))]},
        ],lambda ex:_mf(n_shown+2))
        return {"subtype":"multi_attribute_shape","frames":shown_frames,"answer":answer_state,
                "distractors":distractors,"distractor_strategy":"fallback",
                "tier":tier,"b_estimate":round(random.uniform(*cfg["b_range"]),2),
                "age_range":cfg["age_range"],"n_rules":2,"rule_axes":"grow+binary_alternate",
                "shape_types":"circle","fill_levels":"binary_alternate",
    ...
    # TARGET RENDERER
    # ═══════════════════════════════════════════════════════════════════════════
    def _render_attr_frame(self, canvas, state):
        cx=canvas.width//2; cy=canvas.height//2; half=self.frame_hd//2
        r=int(state["size_frac"]*half)
        self._draw_shape(canvas,state["shape"],state["fill"],
                         cx,cy,r,state.get("rotation",0))

    # ═══════════════════════════════════════════════════════════════════════════
    # DISPATCH ROUTINES
    # ═══════════════════════════════════════════════════════════════════════════
    def generate_item(self, tier):
        return _safe_gen(self._gen_multi_attribute_shape, self._fallback_multi_attribute_shape, tier)
    ...
    def render_frame(self, state, show_question_mark=False):
        ...
        self._render_attr_frame(canvas, state)
        return canvas
