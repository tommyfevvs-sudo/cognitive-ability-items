...
def get_structure(shapes_pool):
    """
    Forces the left and right stems of the primary branch 
    to have different shapes from each other.
    """
    random.shuffle(shapes_pool)
    
    def get_mixed_stack(pool, min_total=1, max_total=3):
    ...
    # Pick TWO DIFFERENT shapes for the left and right stems
    stem_left_shape, stem_right_shape = random.sample(sub, 2)
    
    l_stem = [(stem_left_shape, 1)]
    r_stem = [(stem_right_shape, 1)]

    # Left side sub-branches
    l_bottom_left = get_mixed_stack(sub, 1, 2)
    l_bottom_right = get_mixed_stack(sub, 1, 3)
    
    # Right side sub-branches
    r_bottom_left = get_mixed_stack(sub, 1, 2)
    r_bottom_right = get_mixed_stack(sub, 1, 2)
...
def solve_mobile(node, target_shape):
    ...
    # High iteration count to solve the complex constraints
    for _ in range(60000):
        known_vals = random.sample(range(1, 15), len(known_shapes))
...
def render_mobile_only(draw, canvas, icons, node, x, y):
    ...
    if node.left and node.right:
        ...
        beam_w = max(250, min(raw_width // 2, 520))
...
def stamp_legend(canvas, icons, essential_shapes, target_shape, weights, font):
    ...
    for i, (name, val) in enumerate(items):
        ...
        # Paste the icon centered vertically on y_mid
        canvas.paste(icons[name], (cx - 110, y_mid - (ICON_SIZE[1] // 2)), icons[name])
        
        # Draw the text using "lm" (left-middle) anchor.
        # This tells Pillow to use (cx + 5, y_mid) as the middle of the text's left edge.
        draw.text((cx + 5, y_mid), f"= {val}", fill="black", font=font, anchor="lm")
...
def main():
    ...
    metadata_rows = []
    while i < total:
        root = get_structure(list(SHAPE_COLORS.keys()))
        ...
        if final_weights is None or target_shape not in essential_shapes: 
            continue 
        ...
        file_name = f"item{unique_id}_diff_shape_{branches}_{diff_score}.png"
