...
def stamp_legend(canvas, icons, essential_shapes, target_shape, weights, font):
    ...
    for i, (name, val) in enumerate(items):
        ...
        # Icon placement: Center vertically in the legend row
        icon_y = y_mid - (ICON_SIZE[1] // 2)
        canvas.paste(icons[name], (cx - 110, icon_y), icons[name])
        
        # Text placement: Dynamic centering based on actual font metrics
        text_content = f"= {val}"
        bbox = draw.textbbox((0, 0), text_content, font=font)
        text_h = bbox[3] - bbox[1]
        # Align the text's visual center to the legend's horizontal midpoint
        draw.text((cx + 5, y_mid - (text_h // 2) - bbox[1]), text_content, fill="black", font=font)
