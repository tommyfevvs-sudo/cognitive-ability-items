...
main_folder = os.path.join(script_dir, f"Folding & Cutting rectangle_triangle_fold_{timestamp}")
os.makedirs(main_folder, exist_ok=True)

csv_path = os.path.join(main_folder, "batch_metadata.csv")
batch_id = 1
# Check for max batch ID across existing similar folders if desired, 
# but per-folder CSV starts fresh or handles individual batches within.
...
with open(csv_path, mode='a', newline='') as file:
    ...
    for i in range(1, num_questions + 1):
        # Validation Loop to ensure 5 visually distinct answer options
        max_attempts = 50
        for attempt in range(max_attempts):
            cuts, size, final_mode, final_count = generate_cuts(cut_mode)
            
            def reflect_diag_test(cut):
                return [[0.5 + (0.9 - p[1]), 0.5 + (0.9 - p[0])] for p in cut]
            
            # Pre-calculate answer geometry to check uniqueness
            q_v_cuts_test = [c for c in cuts if any(p[1] >= 0.5 for p in c)]
            q_vertical_test = cuts + [reflect_diag_test(c) for c in q_v_cuts_test]
            
            test_sets = {
                'Correct': q_vertical_test + [[[1.0 - p[0], p[1]] for p in c] for c in q_vertical_test],
                'No-Diag': cuts + [[[1.0 - p[0], p[1]] for p in c] for c in cuts],
                'Half-V': cuts + [[[p[0], 1.0 - p[1]] for p in c] for c in cuts],
                'Stamping': cuts + [[[p[0]-0.4, p[1]] for p in c] for c in cuts],
                'No-Unfold': cuts
            }
            
            unique_signatures = set()
            for key in test_sets:
                points = sorted([tuple(np.round(p, 4)) for cut in test_sets[key] for p in cut])
                unique_signatures.add(str(points))
