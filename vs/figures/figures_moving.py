...
def draw_voxels(ax, coords, color, azim_angle=STANDARD_AZIM):
    ...
    # --- DYNAMIC SHADOW FIX ---
    # The camera (azim_angle) rotates counter-clockwise, but LightSource rotates clockwise.
    # Subtracting azim_angle locks the light securely to the camera's perspective.
...
def get_user_preferences():
    print("--- 3D Shape Generator ---")
    while True:
        try:
            total_items = int(input("How many items would you like to generate? "))
            if total_items > 0: break
            print("Please enter a positive number.")
        except ValueError:
            print("Invalid input. Please enter a valid number.")
            
    print("\nSelect the difficulty tier for these items:")
    print("1. EASY (4 blocks, single color)")
    print("2. MEDIUM (6 blocks, single color)")
    print("3. HARD (8 blocks, single color)")
    print("4. RANDOM/MIXED (A random selection of Easy, Medium, and Hard)")
    ...
    return total_items, choice

def run_generator():
    total_items, tier_choice = get_user_preferences()
    ...
    for i in range(1, total_items + 1):
        current_choice = tier_choice
        if tier_choice == '4':
            current_choice = str(random.randint(1, 3))
            
        if current_choice == '1': 
            tier, num_b, color = "EASY", 4, '#9ACD32' 
        elif current_choice == '2': 
            tier, num_b, color = "MEDIUM", 6, '#4DB6AC'
        else: 
            tier, num_b, color = "HARD", 8, '#F06292'

        rot_tag = "ROTATED" if random.random() < ROTATION_PROBABILITY else "STANDARD"
        item_name = f"{tier}_{i:03d}_{rot_tag}"
        ...
        # Save as looping GIF directly to OUTPUT_DIR
        gif_path = os.path.join(OUTPUT_DIR, f"{item_name}_Base.gif")
        frames[0].save(
            gif_path, 
            save_all=True, 
            append_images=frames[1:], 
            duration=133, # Slower speed (approx 7.5 FPS instead of 10 FPS)
            loop=0,       
            disposal=2    
        )
        ...
        labels = ['A', 'B', 'C', 'D', 'E']
        answer_key.append(f"Item {i:03d} ({tier}) [{rot_tag}]: {labels[correct_pos]}")
        ...
        # Save options strip directly to OUTPUT_DIR
        plt.savefig(os.path.join(OUTPUT_DIR, f"{item_name}_Options.png"), 
                    transparent=True, dpi=300, bbox_inches='tight', pad_inches=0.1)
        plt.close(fig_s)
        print(f"Exported: {item_name} ({i}/{total_items})")
    ...
    print(f"\nGeneration complete! Files saved directly to: {OUTPUT_DIR}")
